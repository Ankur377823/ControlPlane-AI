...
# http://localhost:8000/docs`}
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          The <code>--watch</code> flag leverages Docker Compose File Watch to automatically synchronize changes to backend Python files and frontend React components directly into the container with zero manual rebuilds!
        </p>
      </section>

      <section id="policies-preset" className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white font-brand flex items-center gap-2">
      ...
      <section id="extension-install" className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
        ...
        <ol className="space-y-2 text-sm text-slate-600 dark:text-slate-300 list-decimal list-inside leading-relaxed">
          <li>Open <code>chrome://extensions</code> in Google Chrome and enable <strong className="text-slate-900 dark:text-white font-semibold">Developer mode</strong>.</li>
          <li>Click <strong className="text-slate-900 dark:text-white font-semibold">Load unpacked</strong> and select the directory <code>c:\ControlPlane\frontend\extension</code>.</li>
