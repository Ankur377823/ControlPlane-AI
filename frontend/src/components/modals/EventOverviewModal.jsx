...
import { X, ShieldCheck, RefreshCw, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export function EventOverviewModal({ eventId, sessionId, isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 dark:bg-dark-900/80 backdrop-blur-md animate-fade-in transition-colors">
      <div className="w-full max-w-4xl max-h-[90vh] bg-white dark:bg-dark-850 border border-slate-200 dark:border-primary/40 rounded-3xl shadow-2xl shadow-slate-400/30 dark:shadow-black/80 flex flex-col overflow-hidden transition-colors">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {loading ? (
            ...
            <>
              {/* Event Banner */}
              <div className="p-5 rounded-2xl bg-slate-50 dark:bg-gradient-to-r dark:from-dark-800 dark:to-dark-750 border border-slate-200 dark:border-white/10 space-y-2">
                ...
                <div className="text-xs text-slate-600 dark:text-slate-400 flex flex-wrap gap-4 pt-1">
                  ...
                  <span>
                    Category: <strong className="text-amber-700 dark:text-amber-400">{activeEvent.context}</strong>
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-dark-900/60 border border-slate-200 dark:border-white/10 space-y-2">
                  ...
                  <pre className="text-xs font-mono text-slate-800 dark:text-slate-200 bg-white dark:bg-black/40 p-3.5 rounded-xl border border-slate-200 dark:border-white/5 overflow-x-auto whitespace-pre-wrap">
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-dark-900/60 border border-slate-200 dark:border-white/10 space-y-2">
                  ...
                  <pre className="text-xs font-mono text-emerald-800 dark:text-emerald-200 bg-white dark:bg-black/40 p-3.5 rounded-xl border border-slate-200 dark:border-white/5 overflow-x-auto whitespace-pre-wrap">
              ...
              <div className="p-5 rounded-2xl bg-slate-50 dark:bg-dark-800/80 border border-primary/25 space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 bg-white dark:bg-dark-900 border border-slate-300 dark:border-white/10 rounded-xl px-3.5 py-2 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-primary"
                  />
                  <div className="flex gap-1.5">
                    <button
                      onClick={() => handleFeedback('CONFIRM_FP')}
                      className="px-3 py-2 rounded-xl bg-slate-200/80 hover:bg-slate-300 dark:bg-white/5 dark:hover:bg-white/10 text-slate-800 dark:text-slate-300 border border-slate-300 dark:border-white/10 text-xs font-semibold flex items-center gap-1"
                      title="Mark False Positive"
                    >
                      <ThumbsDown className="w-3.5 h-3.5" /> FP
                    </button>
                    <button
                    ...
                    >
                      <ThumbsUp className="w-3.5 h-3.5" /> TP
                    </button>
                  </div>
                </div>
